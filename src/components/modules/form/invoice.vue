  ...
  form.form(@submit.prevent="")
    .form-item-row
      c-date(
        ...
        :label="labels.invoice.info.dateOfIssue[lang]"
        :error="$v.dateOfIssue.$error"
      )

      c-date(v-model="limit" :label="labels.invoice.info.limit[lang]").element-margin-left

    c-text(
      v-model.trim="$v.projectTitle.$model"
      :label="labels.invoice.info.projectTitle[lang]"
    ...
    c-select(
      ...
      :options="clients"
      :label="labels.invoice.info.selectClient[lang]"
    ...
    .form-item-group.element-margin-top(v-for="(item, i) in items")
      ...
      .form-item-row.form-item-prices
        ...
        .form-item.element-margin-top
          label.form-item__label {{ labels.invoice.info.item.amount[lang] }}
    ...
    div(v-if="tax")
      .form-item.element-margin-top
        label.form-item__label {{ labels.invoice.info.subtotal[lang] }}
        input(:value="formSubtotalAmount" disabled).form-item__text
      .form-item.element-margin-top
        label.form-item__label {{ labels.invoice.info.tax[lang] }}
        input(:value="formTaxAmount" disabled).form-item__text
      .form-item.element-margin-top
        label.form-item__label {{ labels.invoice.info.totalAmount[lang] }}
    ...
    c-textarea(v-model="note" :label="labels.invoice.info.note[lang]").element-margin-top
...
export default {
  ...
  computed: {
    ...
    ...mapState('invoice', ['form', 'invoices']),
    ...mapState('client', ['clients']),
    ...mapGetters('invoice', ['formSubtotalAmount', 'formTaxAmount', 'formTotalAmount']),
    ...mapFields([
